...
SIGNUP_PATHS = [
	"/signup",
	"/api/method/lms.lms.user.sign_up",
	"/api/method/frappe.core.doctype.user.user.sign_up",
	"/api/method/frappe.www.login.sign_up",
	"/api/method/frappe.website.doctype.web_form.web_form.accept",
]
...
def authenticate():
	...
	if frappe.conf.get("private_lms") and (
		path in SIGNUP_PATHS or frappe.form_dict.get("cmd") in [p.split("/api/method/")[-1] for p in SIGNUP_PATHS]
	):
		frappe.throw("Public signup is disabled for this private LMS.", frappe.PermissionError)
