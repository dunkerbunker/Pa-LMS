...
from lms.lms.utils import create_user, get_course_progress, get_lms_app_name
...
def create_course():
	app_name = get_lms_app_name()
	title = f"A guide to {app_name}"
	filters = {"title": title}
	...
	course.update(
		{
			...
			"short_introduction": f"Learn the basics of {app_name} and how to get started with your very first course.",
			"image": "/assets/lms/images/course_card.jpeg",
		}
	)

	course.description = f"""
		This course will cover the fundamentals of {app_name}, including how to create and manage courses, enroll students, and track progress. You will learn about the following key features of the app:
		<br>
		<h3>Key Features</h3>
		<br>
...
def create_intro_lesson_2(course):
	title = f"What is {get_lms_app_name()}?"
	chapter = get_chapter(course, "Introduction")
	content = """
...
def create_quiz():
	...
	questions.append(
		create_quiz_questions(
			f"What is {app_name} primarily used for?",
			"Project Management",
			False,
			"Learning Management",
			True,
	...
	questions.append(
		create_quiz_questions(
			f"Which of the following can be added to a course in {app_name}?",
			"Lessons",
			True,
			"Issues",
			False,
	...
	questions.append(
		create_quiz_questions(
			f"What is the top-level structure in {app_name}?", "Chapter", False, "Course", True
		)
	)
	questions.append(
		create_quiz_questions(f"Can you create quizzes in {app_name}?", "Yes", True, "No", False)
	)
	questions.append(
		create_quiz_questions(
	...
	questions.append(
		create_quiz_questions(f"Can you track learner progress in {app_name}?", "Yes", True, "No", False)
	)
	questions.append(
		create_quiz_questions(
			f"What is the purpose of a batch in {app_name}?",
			"To group learners",
			True,
	...
	questions.append(
		create_quiz_questions(
			f"How can you create custom certificates in {app_name}?",
...
def create_reviews(course, student):
	...
	review.rating = 0.8
	review.review = f"This is a great course to get started with {get_lms_app_name()}. The content is well-structured and easy to follow."
	review.save()
		...
		"blocks": [
			{
				...
				"type": "paragraph",
				"data": {
					"text": f"{app_name} allows you to embed videos in lessons using popular video hosting platforms."
				},
			},
			{
				"id": "1ooWPn5Zmq",
				"type": "paragraph",
				"data": {
					"text": f"You don't need to upload videos directly into {app_name} - simply copy the video URL from your preferred provider and paste it into the Lesson Editor."
				},
			},
			{
				"id": "tCJD0yMAGd",
				"type": "paragraph",
				"data": {
					"text": f"{app_name} automatically detects the video source and embeds it for learners."
				},
			},
