...
def verify_billing_access(doctype, name, billing_type):
	...
	if is_private_lms():
		return False, _("Enrollment is invite-only.")
...
def get_job_details(job: str):
	frappe.throw(_("The job board is disabled."), frappe.PermissionError)
...
def get_branding():
	...
	settings.app_name = get_lms_app_name()
	return settings
...
def delete_course(course: str):
	...
	if frappe.db.get_value("LMS Course", course, "published"):
		frappe.throw(_("Unpublish the course before deleting it."))
...
def get_lms_settings():
	...
	settings.private_lms = cint(frappe.conf.get("private_lms") or 0)
	settings.app_name = get_lms_app_name()
...
def _normalise_roles(roles: list[str] | str | None = None):
	if isinstance(roles, str):
		roles = json.loads(roles or "[]")
	roles = roles or ["LMS Student"]
	allowed = set(LMS_ROLES)
	out = []
	for role in roles:
		if role not in allowed:
			frappe.throw(_("You do not have permission to assign role {0}.").format(role), frappe.PermissionError)
		if role not in out:
			out.append(role)
	if "LMS Student" not in out:
		out.append("LMS Student")
	return out


def _normalise_courses(courses: list[str] | str | None = None):
	if isinstance(courses, str):
		courses = json.loads(courses or "[]")
	courses = courses or []
	out = []
...
def _invite_url(token: str) -> str:
	return frappe.utils.get_url(get_lms_route(f"invite/{token}"))


def _send_invite_email(invite, token: str) -> bool:
	try:
		frappe.sendmail(
			recipients=[invite.email],
			subject=_("You're invited"),
			message=_("Use this secure invite link to complete your account setup: {0}").format(_invite_url(token)),
			retry=3,
		)
		return True
	except frappe.OutgoingEmailError:
		frappe.log_error(
			title="Unable to send LMS invite email",
...
def create_invite(
	...
	courses = _normalise_courses(courses)

	active_invite = frappe.db.exists("LMS User Invite", {"email": email, "status": "Pending"})
	if active_invite:
		invite = frappe.get_doc("LMS User Invite", active_invite)
		invite.status = "Revoked"
		invite.save(ignore_permissions=True)

	token = frappe.generate_hash(length=48)
	invite = frappe.new_doc("LMS User Invite")
	invite.email = email
	invite.first_name = first_name
	invite.last_name = last_name
	invite.status = "Pending"
	invite.token_hash = _invite_token_hash(token)
	invite.expires_at = add_days(now_datetime(), cint(frappe.conf.get("lms_invite_expiry_days") or 7))
	invite.invited_by = frappe.session.user
	invite.roles_json = json.dumps(roles)
	for course in courses:
		invite.append("courses", {"course": course})
	invite.insert(ignore_permissions=True)
	email_sent = _send_invite_email(invite, token)
	return {
		"name": invite.name,
		"email": invite.email,
...
@frappe.whitelist()
def get_pending_invites(start: int = 0, search: str = None):
	frappe.only_for("Moderator")
	filters = {"status": "Pending"}
	or_filters = {}
	if search:
		or_filters["email"] = ["like", f"%{search}%"]
		or_filters["first_name"] = ["like", f"%{search}%"]
		or_filters["last_name"] = ["like", f"%{search}%"]
	return frappe.get_all(
		"LMS User Invite",
		filters=filters,
		or_filters=or_filters,
		fields=["name", "email", "first_name", "last_name", "expires_at", "invited_by", "creation"],
		order_by="creation desc",
		page_length=20,
		start=start,
	)
...
@frappe.whitelist()
def resend_invite(invite_name: str):
	frappe.only_for("Moderator")
	invite = frappe.get_doc("LMS User Invite", invite_name)
	if invite.status != "Pending":
		frappe.throw(_("Only pending invites can be resent."), frappe.PermissionError)
	token = frappe.generate_hash(length=48)
	invite.token_hash = _invite_token_hash(token)
	invite.expires_at = add_days(now_datetime(), cint(frappe.conf.get("lms_invite_expiry_days") or 7))
	invite.save(ignore_permissions=True)
...
@frappe.whitelist()
def revoke_invite(invite_name: str):
	frappe.only_for("Moderator")
	invite = frappe.get_doc("LMS User Invite", invite_name)
	if invite.status != "Pending":
		frappe.throw(_("Only pending invites can be revoked."), frappe.PermissionError)
	invite.status = "Revoked"
	invite.save(ignore_permissions=True)
	return True
...
def _get_pending_invite(token: str):
	if not token:
		frappe.throw(_("Invite token is required."), frappe.PermissionError)
	rows = frappe.db.sql(
		"select name from `tabLMS User Invite` where token_hash = %s for update",
		_invite_token_hash(token),
		as_dict=True,
	)
	if not rows:
		frappe.throw(_("This invite is invalid, expired, or already used."), frappe.PermissionError)
	invite = frappe.get_doc("LMS User Invite", rows[0].name)
	invite.validate_acceptance()
	return invite


@frappe.whitelist(allow_guest=True)
def get_invite(token: str):
	invite = _get_pending_invite(token)
	return {
		"email": invite.email,
		"first_name": invite.first_name,
		"last_name": invite.last_name,
		"expires_at": invite.expires_at,
		"courses": [row.course for row in invite.courses],
	}


def _ensure_invited_user(invite, first_name: str, last_name: str):
	roles = _normalise_roles(invite.roles_json)
	if frappe.db.exists("User", invite.email):
		user = frappe.get_doc("User", invite.email)
		user.enabled = 1
		user.first_name = first_name or user.first_name or invite.first_name or invite.email.split("@")[0]
		user.last_name = last_name or user.last_name or invite.last_name
		...
		user = frappe.get_doc(
			{
				"doctype": "User",
				"email": invite.email,
				"first_name": first_name or invite.first_name or invite.email.split("@")[0],
...
def _assign_courses(user: str, courses: list[str]):
	for course in courses:
		...
		enrollment.flags.ignore_private_lms_assignment = True
		enrollment.insert(ignore_permissions=True)


@frappe.whitelist(allow_guest=True)
def accept_invite(token: str, first_name: str, last_name: str = None, password: str = None):
	if not password:
		frappe.throw(_("Password is required."))
	invite = _get_pending_invite(token)
	user = _ensure_invited_user(invite, first_name, last_name)
	update_password(user.name, password, logout_all_sessions=True)
	courses = [row.course for row in invite.courses]
	_assign_courses(user.name, courses)
	invite.status = "Accepted"
	invite.used_at = now_datetime()
	invite.used_by = user.name
	invite.invited_user = user.name
	invite.save(ignore_permissions=True)
	return {"user": user.name}


@frappe.whitelist()
def get_user_course_assignments(user: str):
	frappe.only_for("Moderator")
	return frappe.get_all("LMS Enrollment", {"member": user}, pluck="course")


@frappe.whitelist()
def update_user_course_assignments(user: str, courses: list[str] | str | None = None):
	frappe.only_for("Moderator")
	courses = set(_normalise_courses(courses))
	current = set(frappe.get_all("LMS Enrollment", {"member": user}, pluck="course"))
	_assign_courses(user, list(courses - current))
	for course in current - courses:
		frappe.delete_doc(
			"LMS Enrollment",
			frappe.db.get_value("LMS Enrollment", {"member": user, "course": course}, "name"),
...
def clear_demo_data():
	frappe.only_for("Moderator")
	quiz_titles = ["Do you know Frappe Learning?", f"Do you know {get_lms_app_name()}?"]
	...
	demo_course = frappe.get_all(
		"LMS Course",
		{"title": ["in", ["A guide to Frappe Learning", f"A guide to {get_lms_app_name()}"]]},
		pluck="name",
	)
