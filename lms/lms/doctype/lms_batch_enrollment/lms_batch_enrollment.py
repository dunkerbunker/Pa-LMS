...
class LMSBatchEnrollment(Document):
	...
	def validate(self):
		self.validate_private_lms_assignment()
		self.validate_owner()
	...
	def validate_private_lms_assignment(self):
		if not frappe.conf.get("private_lms"):
			return
		if self.flags.ignore_private_lms_assignment:
			return
		if self.is_admin():
			return
