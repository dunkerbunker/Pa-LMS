...
def has_website_permission(doc, ptype, user, verbose=False):
	if frappe.conf.get("private_lms"):
		...
		is_admin = bool({"Moderator", "Course Creator", "Batch Evaluator", "System Manager"} & set(roles))
		return is_admin or doc.member == user
...
def get_permission_query_conditions(user):
	...
	if "Moderator" in roles or "Course Creator" in roles or "Batch Evaluator" in roles:
		return None
	if frappe.conf.get("private_lms"):
		return """(`tabLMS Certificate`.`member` = {user})""".format(user=frappe.db.escape(user))
