...
from lms.lms.utils import get_lms_app_name


class LMSSettings(Document):
	def validate(self):
		self.apply_private_lms_config()
		self.validate_google_settings()
		self.validate_signup()
		self.validate_contact_us_details()
		self.validate_lesson_dwell_time()

	def apply_private_lms_config(self):
		if frappe.conf.get("private_lms"):
			self.allow_guest_access = 0
			self.disable_signup = 1
			frappe.db.set_single_value("Website Settings", "app_name", get_lms_app_name())
