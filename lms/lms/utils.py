...
def is_private_lms():
	return cint(frappe.conf.get("private_lms") or 0)


def get_lms_app_name():
	return (
		frappe.conf.get("lms_app_name")
		or os.environ.get("LMS_APP_NAME")
		or DEFAULT_LMS_APP_NAME
	)
...
def guest_access_allowed():
	if is_private_lms():
		return False
	if frappe.conf.get("allow_guest_access") is not None:
		return bool(cint(frappe.conf.get("allow_guest_access")))
...
@frappe.whitelist(allow_guest=True)
@rate_limit(limit=500, seconds=60 * 60)
def get_courses(filters: dict | str | None = None, start: int = 0) -> list:
	...
	if not filters:
		filters = {}
	if is_private_lms() and not has_moderator_role():
...
def get_course_categories() -> list:
	...
	if is_private_lms() or not guest_access_allowed():
		return []
...
def get_course_details(course: str):
	...
	if is_private_lms() and not (membership or can_modify_course(course)):
		return {}
...
def get_course_outline(course: str, progress: bool = False) -> list:
	...
	if frappe.session.user == "Guest" and not guest_access_allowed():
		return []
	if is_private_lms() and not (get_membership(course) or can_modify_course(course)):
		return []
...
def get_batch_details(batch: str):
	...
	if is_private_lms() and not (is_batch_admin or is_student_enrolled):
		return {}
...
def get_order_summary(doctype: str, docname: str, coupon: str | None = None, country: str | None = None):
	if is_private_lms():
		if doctype == "LMS Course" and not (get_membership(docname) or can_modify_course(docname)):
			frappe.throw(_("You are not authorized to view this course."), frappe.PermissionError)
		if doctype == "LMS Batch":
			is_student = frappe.db.exists(
				"LMS Batch Enrollment", {"batch": docname, "member": frappe.session.user}
			)
			if not (is_student or can_modify_batch(docname)):
				frappe.throw(_("You are not authorized to view this batch."), frappe.PermissionError)
...
def enroll_in_batch(batch: str, payment_name: str = None):
	if is_private_lms():
		frappe.throw(_("Batch enrollment is invite-only."), frappe.PermissionError)
	if not frappe.db.exists("LMS Batch", batch):
		frappe.throw(_("The specified batch does not exist."))
...
@frappe.whitelist()
def get_programs():
	if not guest_access_allowed() and frappe.session.user == "Guest":
	...
	published_programs = []
	if not is_private_lms() or has_moderator_role() or has_course_instructor_role():
...
def get_program_details(program_name: str) -> dict:
	...
	if is_private_lms() and not (is_member or has_moderator_role() or has_course_instructor_role()):
...
def enroll_in_program(program: str):
	if is_private_lms():
		frappe.throw(_("Program enrollment is invite-only."), frappe.PermissionError)
...
def get_batches(filters: dict = None, start: int = 0, order_by: str = "start_date"):
	...
	if not filters:
		filters = {}
	if is_private_lms() and not (has_moderator_role() or has_evaluator_role()):
		filters["enrolled"] = 1
...
def get_related_courses(course: str) -> list:
	if is_private_lms() and not can_modify_course(course):
		return []
	if not guest_access_allowed():
		return []
...
def is_demo_course(course: str) -> bool:
	...
	return title in ["A guide to Frappe Learning", f"A guide to {get_lms_app_name()}"]
