...
from lms.lms.utils import get_lms_app_name, get_lms_path, get_lms_route
...
def get_context():
	...
	if frappe.conf.get("private_lms") and frappe.session.user == "Guest" and not is_public_app_path(app_path):
		redirect_to = f"?redirect-to=/{get_lms_path()}/{app_path}" if app_path else ""
		frappe.redirect(f"/login{redirect_to}")
...
def is_public_app_path(app_path: str | None):
	return bool(app_path and app_path.startswith("invite/"))
